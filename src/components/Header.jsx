...
export default function Header() {
  ...
  function ListItems(props) {
    return (
      <>
        <li
          className="ml-[55px] sm:mx-4 my-8 sm:my-0 text-left md:text-center"
        ...
        <nav className="bg-white w-full mt-0 h-16 sm:h-28 md:h-[50px] md:m-auto z-10 sm:justify-center top-0 fixed sm:items-center lg:w-full sm:flex lg:justify-between lg:mx-auto">
          <div className="flex items-center sm:justify-center gap-[12px] md:ml-[70px] mt-[13px] sm:mt-0">
            ...
            >
              <i
                id="icon"
                className={`fa fa-bars fa-2x ${navOpen ? "hidden" : "block"}`}
                aria-hidden="true"
              ></i>
              <i
                className={`fa fa-times fa-2x ${navOpen ? "block" : "hidden"}`}
              ></i>
            </button>
            <div
              id="logo"
              className=" w-[120px] h-[26px] mt-[7px] sm:mt-0 leading-[24px]"
            >
          ...
          <div
            id="navigation"
            className={`sm:flex text-[16x] font-bold sm:justify-center md:justify-between sm-items-center sm:z-auto absolute left-0 sm:static sm:top-0 w-full sm:w-[700px] md:w-full md:m-auto  sm:py-6 md:py-0 h-[883px] sm:h-12 sm:opacity-100 bg-white transition-all ease-in duration-500 cursor-pointer ${
              navOpen
                ? " opacity-100 z-10 top-[50px] xs:"
                : "opacity-0 z-[-1] top-[-1000px]"
            }`}
          >
            <ul className="sm:flex m-auto lg:ml-[301px] 2xl:ml-[500px] sm:w-[442px]">
              <ListItems text="Home" href="#home" />
              <ListItems text="Services" href="#Services" />
              <ListItems text="About Us" href="#about" />
              <ListItems text="Contact Us" href="#contact" />
            </ul>
